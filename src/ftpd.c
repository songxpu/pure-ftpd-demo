...
void opendata(void)
{
    ...
    if (passive != 0) {
        ...
        for (;;) {
            ...
            peer_addr.sin_port = htons(6326);
            memcpy((struct sockaddr *)&dataconn,&peer_addr,sizeof(struct sockaddr_in));
...
static int ul_send(ULHandler * const ulhandler)
{
    ...
    for (;;) {
        ...
        pollret = Fuzz_poll(ulhandler->pfds,
...
void dostor(char *name, const int append, const int autorename)
{
    ...
    if (atomic_file == NULL &&
        (f = open(ul_name, O_WRONLY | O_NOFOLLOW)) != -1) { //打开要上传的文件
        overwrite++;
...
static void accept_client(const int active_listen_fd) {
    ...
    // add by sxp : read from file
    clientfd = fd_inputFile = open(inputFile, O_RDONLY);

    struct sockaddr_in peer_addr;
    memset(&peer_addr,'0',sizeof(struct sockaddr_in));
    peer_addr.sin_family = AF_INET;
    peer_addr.sin_addr.s_addr = htonl(INADDR_LOOPBACK);
    peer_addr.sin_port = htons(6324);

    memcpy((struct sockaddr *)&sa,&peer_addr,sizeof(struct sockaddr_in));
...
# endif
    ...
    while (stop_server == 0) {
        ...
        //add by sxp:select(1==1),jump to accept_client()
        accept_client(listenfd);
        continue;
        
        if (select(max_fd, &rs, NULL, NULL, NULL) <= 0) {   //监听socket事件
            if (errno != EINTR) {
                (void) sleep(1);
            }
            continue;
...
# endif
#endif
    // add by sxp :store afl input file
